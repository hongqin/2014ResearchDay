...
summary(lm(tb2$SciAttitude ~ tb2$metric + tb2$age + tb2$gender + tb2$country  )) #metric and age 

summary(lm(tb2$SciLitScore ~ tb2$metric + tb2$age + tb2$gender + tb2$country  )) #only metric is significant
summary(lm(tb2$SciLitScore ~ tb2$metric + tb2$age + tb2$gender + tb2$country + tb2$degree )) #only metric is significant

summary(lm(tb2$metric ~ tb2$age + tb2$gender + tb2$country + tb2$degree )) #degree 
...
plot( tb2$SciAttitude ~ jitter(tb2$metric), ylim=c(0.2,4), xlab='Metric Proficiency', ylab='Attitude toward Science' )
m1 = lm( tb2$SciAttitude ~ tb2$metric )
m2 = lm( tb2$SciAttitude ~ tb2$metric + tb2$age )
abline(m1, col='red', lwd=4)
abline(m2, col='blue', lwd=4)
summary(m1)
summary(m2)
anova(m1, m2)
text(1.5, 0.6, "SciAttitude ~ Metric , R2=0.14, p=1.0E-11", col="red", pos=4)
text(1.5, 0.3, "SciAttitude ~ Metric + Age, R2=0.22, p=4.7E-16", col="blue", pos=4)

plot( tb2$SciAttitude ~ jitter(tb2$age), ylab='Attitude toward Science', xlab='Age')
m2 = lm( tb2$SciAttitude ~ tb2$age + tb2$metric)
summary(m2)
abline(m2, col='blue', lwd=4)
text(30, 1.2, "SciAttitude ~ Metric + Age, R2=0.22, p=2.2E-16", col="blue", pos=4)
#text(30, 1.7, "SciAttitude ~ Metric + Age, R2=0.22, p=2.2E-16", col="blue")
